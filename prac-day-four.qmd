...
```{r}
##Displaying the 'flights' dataset in an alternative format:
...
dist_tbl <- flights |>
  filter(month == 1) |>
  group_by(carrier) |>
  summarise(mean_distance = mean(distance, na.rm = TRUE), sd_distance = sd(distance, na.rm = TRUE)) |> arrange(mean_distance)
...
```{r}
#Carriers that have a sd of NA have only completed one flight and hence there is no deviation as there is only one flight, therefore the totall distance must equal the mean distance

sd_na_flights <- flights |>
  filter(month == 1) |>
  group_by(carrier) |>
  summarise(total_dist = sum(distance, na.rm = TRUE),
            mean_dist = mean(distance, na.rm = TRUE))|>
  filter(total_dist == mean_dist)
print(sd_na_flights)
```

```{r}
#Carriers that have a sd of 0 only complete flights of the same distance hence the sd is 0 as there is no devation away from the mean. 

sd_0_na_flights <- flights |>
  filter(month == 1) |>
  group_by(carrier) |>
  summarise(min_dist = min(distance, na.rm = TRUE),
            max_dist = max(distance, na.rm = TRUE)) |>
  filter(min_dist == max_dist)

sd_0_flights <- sd_0_na_flights |>
  anti_join(sd_na_flights, by = "carrier")
print(sd_0_flights)
```
...
```{r}
#carriers are along the columns, and the rows are the average departure delay (dep_delay) flown by each carrier (carrier) in each month.
...
proportion_flights <- number_flights/total_flights  
...
common_routes <- flights |>
  ...
  summarise(unique_airlines = n_distinct(carrier), .groups = "drop") |>
  filter(unique_airlines > 1)
print(common_routes)
#add names
```
...
```{r}
ave_arrival_delay_common_routes <- flights |>
  semi_join(common_routes, by = c("origin", "dest")) |>
  group_by(origin, dest, carrier) |>
  summarise(ave_arr_delay = mean(arr_delay, na.rm = TRUE))
print(ave_arrival_delay_common_routes)

named_ave_arrival_delay_common_routes <- ave_arrival_delay_common_routes |> 
...
```{r}
route_performance <- named_ave_arrival_delay_common_routes |>
  group_by(origin, dest) |>
  summarise(
    best_airline = name[which.min(ave_arr_delay)],
    smallest_delay = min(ave_arr_delay, na.rm = TRUE),
    worst_airline = name[which.max(ave_arr_delay)],
    longest_delay = max(ave_arr_delay, na.rm = TRUE),
    .groups = "drop"
  )
...
```{r}
biggest_diff <- route_performance |>
  mutate(delay_difference = longest_delay - smallest_delay) |>
  arrange(desc(delay_difference)) |>
  slice(1)
print(biggest_diff)
```
...
```{r}
#| echo: false
question_seven <- structure(list(id = c("id_1", "id_2", "id_3", "id_4", "id_5", 
...
```{r}
UtilsDataRSV::view_cols(question_seven)
```
